...
- This part is for reading in all related data frames, cleaning them, and extracting all data from the 2000-1 - 2022-02 time period
...
- Short term forecast horizon is determined to be 3 months since it might be helpful for short term energy usage planning
- Long term forecast horizon is determined to be 36 months, since it is helpful to forecast pricing a few years out for infrastructure and budget planning

```{r}
# There seem to be a slowly damping behavior which might support differencing the data
...
# By using the overfitting method, 1-B term seems to have the largest absolute reciprocal root which by itself supports differencing the data
# Additionally, there seems to be a 1-B^12 seasonality, even though some of the factors such as 1+B at system frequency of -.5 and 1+1.73B+1B^2 at System frequency of 0.4167 are not as close to the unit circle. It might be worth exploring s = 12 also.
...
- This section created a signal plus noise model. However, it was not utilized as the final model during the presentations
...
summary(fit)
# As can be seen, after accounting for the serial correlation (AR(14)), there is strong evidence to suggest that the slope is significantly different from zero (pvalue < 0.0001). So we will take the trend out and proceed
